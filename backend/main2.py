...
from contextlib import asynccontextmanager  # Removed: Not used in the code
import csv
...
import pandas as pd
from prometheus_client import start_http_server, Summary, Gauge, Counter, Histogram, REGISTRY
import time
import logging
import requests
...
def load_model(model_version):
    try:
        global model, scaler, merchant_encoder, category_encoder, gender_encoder, job_encoder, latest_model_file, explainer
...
def data_drift():
    try:
        ...
        numerical_features = ['amt', 'unix_time', 'lat', 'long', 'merch_lat', 'merch_long']
        categorical_features = ['category', 'gender', 'state', 'job']
        
        drift_scores = {}

        # Numerical features drift detection (KS test)
        ...
        # Categorical features drift detection (Chi-Square test)
        for feature in categorical_features:
            # Build contingency table for categorical features
            contingency_table = pd.crosstab(train_df[feature], test_df[feature], dropna=False)
        ...
        for feature, scores in drift_scores.items():
            if scores["p_value"] < 0.05:
                ...
                description = f"Significant data drift detected in feature '{feature}' with statistic {scores['ks_statistic' if 'ks_statistic' in scores else 'chi2_statistic']:.4f} and p-value {scores['p_value']:.4f}."
    ...
    try:
        ...
        for log in logs:
            try:
                ...
                if start_date:
                    transaction_time = datetime.datetime.fromisoformat(transaction.get("timestamp"))
                    if transaction_time < datetime.datetime.fromisoformat(start_date):
                        continue
                if end_date:
                    transaction_time = datetime.datetime.fromisoformat(transaction.get("timestamp"))
                    if transaction_time > datetime.datetime.fromisoformat(end_date):
                        continue
        ...
        return {"filtered_transactions": paginated_transactions}
    except FileNotFoundError as fe:
        logging.error(str(fe))
        raise HTTPException(status_code=500, detail=str(fe))
    except Exception as e:
...
@app.post("/train")
async def train(
    n_estimators: Optional[int] = Query(100, description="Number of trees in the forest"),
    max_depth: Optional[int] = Query(None, description="Maximum depth of the tree"),
    min_samples_split: Optional[int] = Query(2, description="Minimum number of samples required to split a node"),
    min_samples_leaf: Optional[int] = Query(1, description="Minimum number of samples required to be at a leaf node"),
    random_state: Optional[int] = Query(42, description="Random state for reproducibility")
):
    # Call the train_model function with the provided parameters
    metrics = train_model(
...
async def generate_shap_plots_endpoint(model_name: str, dependence_feature: str = "amt"):
    try:
        ...
        X_test = preprocess_dataset_for_shap(test_data.drop(columns=["is_fraud"], errors='ignore'))
        ...
        # If preprocess_dataset_for_shap drops columns, ensure they are handled
        # Since preprocess_dataset_for_shap drops columns, features are already numeric codes
